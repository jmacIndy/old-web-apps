...
<html>
   ...
   <body>
      ...
      <?php
         ...
         $result = mysql_query("SELECT * FROM PEM_TREND ORDER BY PEM_TREND_ID DESC", $db);
         echo("<table border=1>\n");
         echo("<tr><td>Trend ID</td><td>Record Type</td><td>Acceptance Criteria</td><td>Site Identifier</td><td>Sample Date</td>");
         echo("<td>Created Date</td><td>Trend Status</td><td>Gram +/-</td></tr>\n");
         while ($myrow = mysql_fetch_array($result))
         {
            if ( $myrow['TREND_RCRD_TYP_CD'] == 'Microflora' )
            {
               $trendType = $myrow['TREND_RCRD_TYP_CD'] . ' (' . $myrow['TREND_GENERE_DESC'] . ')';
            }
            else
            {
               $trendType = $myrow['TREND_RCRD_TYP_CD'];
            }
            printf('<tr><td><a href="incidentTrend.php?trendId='.$myrow[0].'">%s</a></td>', $myrow['PEM_TREND_ID']);
            printf("<td>%s</td><td>%s</td><td>%s</td><td>", $trendType, 
               $myrow['TREND_ACPTNC_CRITERIA_DESC'], $myrow['TREND_SITE_DESC']);
            print date('Y-m-d h:i:s A', strtotime($myrow['TREND_SMPL_DT']));
            printf("</td><td>");
            print date('Y-m-d h:i:s A', strtotime($myrow['TREND_CRTD_DT']));
            ...
            printf("</td><td>%s</td><td>%s</td></tr>\n", $trendStatus, $myrow['TREND_GRAM_POS_NEG_CD']);
         }
         echo("</table>\n");
         mysql_close($db);
      ?>
